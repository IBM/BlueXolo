...
def run_keyword(host, user, passwd, filename, script, values):
    ...
    ssh.send_results_named(host, user, passwd, result)
    return result
...
class SshConnect(LoginRequiredMixin):
    ...
    def run_file_named(self, filename, host, user, passwd):
        ...
        run_path = 'cd /home/tipadmin/Desktop/Pruebas/PruebaKeyword'
    ...
    def send_results_named(self, host, user, passwd, filename):
        ...
        scp = SCPClient(t)
        scp.get('/home/tipadmin/Desktop/Pruebas/PruebaKeyword/{0}_log.html'.format(filename),
                '{0}/test_result/'.format(settings.MEDIA_ROOT))
        scp.get('/home/tipadmin/Desktop/Pruebas/PruebaKeyword/{0}_report.html'.format(filename),
                '{0}/test_result/'.format(settings.MEDIA_ROOT))
        scp.get('/home/tipadmin/Desktop/Pruebas/PruebaKeyword/{0}_output.xml'.format(filename),
                '{0}/test_result/'.format(settings.MEDIA_ROOT))
        scp.close()
        t.close()
